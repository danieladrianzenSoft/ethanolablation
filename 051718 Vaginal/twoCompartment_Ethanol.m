...
[D_OH,D_S,h_OH,h_S,phi,C_0,t] = initializeParameters();
...
[t2,C] = ode15s(@(t2,C) dcdt(t,C,x,h_OH,h_S,D_OH,D_S,phi,nOH,ns,C_0), t, IC,opts1);

C = C';

PsurfaceC = C(1,:)/mean(C(1,:))*1.13;
%measurements(:,1)
%round(t2/60)
...
function [D_OH,D_S,h_OH,h_S,phi,C_0,t] = initializeParameters()
...
thickness = [1.3,1.7,1.4,1.2,1.6,2.0,2.3,1.6,1.8,1.5];
h_S = mean(thickness/10) %convert cm to mm
h_OH = 1.5; %cm
t = linspace(0,166*60,1001);

end



function DCDT = dcdt(t,C,x,h_OH,h_S,D_OH,D_S,phi,nOH,ns,C_0)
