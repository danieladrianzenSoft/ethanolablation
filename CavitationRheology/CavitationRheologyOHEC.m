...
%% CHANGING OMEGA (ETHYL CELLULOSE CONCENTRATION)

omegaVec = [1, 1-omega, 1-2*omega, 1-4*omega];

%% FINDING STRAIN RATE, PRESSURES AND CAVITY RADIUS

tt = zeros(length(t),length(omegaVec));
lambda = zeros(length(t),length(omegaVec));


for i = 1:length(omegaVec)
    [t_temp,lambda_temp] = ode23s(@(tt,lambda) dLdt(tt,lambda,Q_needle,E,mu_oh,L_needle,r_needle,r0,K_oh,omegaVec(i),Rtot), t, y0);
    tt(:,i) = t_temp;
    lambda(:,i) = lambda_temp;
end

%[tt,lambda] = ode45(@(tt,lambda) dLdt(tt,lambda,Q_needle,E,mu_oh,L_needle,r_needle,r0,K_oh,omega,Rtot), tspan, y0);
...
if (show_strain_radius == 1)
    plotCavityStrainRadius(tt, lambda, cavity_radius, 'o', omegaVec)
end
if (show_pressures == 1)
    plotPressures(tt, lambda, Pc, Pup, Pcrit, 'o', omegaVec)
end

% % function ind_cavity_bound = calcCavityBoundIndex(r, cavity_radius)
% % 
% %     ind_cavity_bound = zeros(1,length(tt)); % vector that will contain index in r of cavity radius
% % 
% %     for jj = 1:length(ind_cavity_bound)
% %        ind_cavity_bound(jj) = binarySearchBin(r,cavity_radius(jj));
% %     end
% % 
% % end
% % 
% % function velocity = calcVelocity(r, t, k_c, k_t, delP_c, delP_t, sigma, r_0, dlambdadt, ind_cavity_bound)
% %     velocity = zeros(length(r), length(t));
% %     
% %     
% %     velocity(1:r())
% % end
% % 
% % function pressure = calcPressureAndGradients(r, t, k_c, k_t, delP_c, delP_t, sigma, r_0, dlambdadt, ind_cavity_bound)
% %     pressure = zeros(length(r),length(t));
% % end

function DLAMBDADT = dLdt(t,lambda,Q_needle,E,mu,L_needle,r_needle,r0,k_T,omega,a)

        Pc = E*(5/6-(2./(3*lambda))-(1./(6*(lambda.^4))));
        DLAMBDADT = (Q_needle./(4*pi*(r0^3)*(lambda.^2)))-((k_T*Pc*omega./((r0^3).*(lambda.^2))).*(a./((a./(lambda*r0))-1)));
        %DLAMBDADT = ((Q_needle*(1-omega))./(4*pi*(r0^3)*(lambda.^2)));
...
%         elseif (jj > 2 && jj == nt_injection_end)
%             C_OH(jj,:) = Ct_oh(end,:);
%         end
%         
