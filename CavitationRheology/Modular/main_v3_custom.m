...
% variables, and cell array for helper functions. Uses custom solver.
...
for i = 1:length(phiVec)
    ...
    %CN SOLVER WITH JACOBIAN:
    [t_injection, c_injection] = crankNicolsonImplicitSolver(@(t_injection,C_injection) getConcentration.velocity_bc_cavity_radius_concentration_with_jacobian(t_injection, C_injection, r, params, helpers), IC_oh, t_injection_span, dt_injection, options);
    ...
    % CN SOLVER WITH JACOBIAN
    [t_relaxation, c_relaxation] = crankNicolsonImplicitSolver(@(t_relaxation,C_relaxation) getConcentration.velocity_bc_cavity_radius_concentration_with_jacobian(t_relaxation, C_relaxation, r, params, helpers), IC_relaxation, t_relaxation_span, dt_relaxation, options);
    % v1.0:
...
for i = 1:length(phiVec)
    ...
    for j = 1:length(tt_vec)
        ...
        [strain_rate_temp, jacobian_temp] = getStrainRate.concentration_strain_rate_porosities_jacobian(r, C_vec(j,:)', C_vec(j,ind_r_cavity_value)', ind_r_cavity_value, lambda_vec(j), r_needle, kappa, Q_needle, C_oh_inj, epsilon_c_vec(i), epsilon_t);
        dlambdadt_vec(j) = strain_rate_temp;
    end
