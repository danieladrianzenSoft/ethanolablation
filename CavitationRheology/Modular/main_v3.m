...
% approach of ode15s. Adjusted to use params cell array instead of free
% variables, and cell array for helper functions.
...
helpers = cell(4,1);
%helpers{1} = @fastFind.binarySearchBin;
%helpers{2} = @getVelocity.ethanol_velocity_porosity;
%helpers{3} = @getVelocityGradient.simple_velocity_gradient_porosity;
%helpers{4} = @getStrainRate.concentration_strain_rate_porosities;
%helpers{4} = @getStrainRate.concentration_strain_rate_porosities_jacobian;
helpers{1} = @fastFind.binarySearchBin;
helpers{2} = @getVelocity.velocity_newtonian_fluid_plus_solid;
helpers{3} = @getVelocityGradient.velocity_gradient_newtonian_fluid_plus_solid;
helpers{4} = @getStrainRate.strain_rate_newtonian_fluid_plus_solid;
%% indices:
...
for i = 1:length(phiVec)
    ...
    params("final_ind_r_cavity") = final_ind_r_cavity;
    % v3.0:
    ...
    % v2.0:
    params("Q_needle") = Q_needle;
    params("D_C") = D_C_vec(i);
    params("theta") = theta_vec(i);
    params("P") = P_vec(i);
    params("epsilon_c") = epsilon_c_vec(i);
    %OG:
    ...
    [t_injection, c_injection] = ode15s(@(t_injection,C_injection) getConcentration.velocity_bc_cavity_radius_concentration_params(t_injection, C_injection, r, params, helpers), t_injection_span, IC_oh, opts1);
    %WITH JACOBIAN:
    %[t_injection, c_injection] = ode15s(@(t_injection,C_injection) getConcentration.velocity_bc_cavity_radius_concentration_with_jacobian(t_injection, C_injection, r, params, helpers), t_injection_span, IC_oh, opts1);
    ...
    % DESTRUCTURING PARAMS:
    [t_relaxation, c_relaxation] = ode15s(@(t_relaxation,C_relaxation) getConcentration.velocity_bc_cavity_radius_concentration_params(t_relaxation, C_relaxation, r, params, helpers), t_relaxation_span, IC_relaxation, opts1);
    % WITH JACOBIAN:
    %[t_relaxation, c_relaxation] = ode15s(@(t_relaxation,C_relaxation) getConcentration.velocity_bc_cavity_radius_concentration_with_jacobian(t_relaxation, C_relaxation, r, params, helpers), t_relaxation_span, IC_relaxation, opts1);
    % v1.0:
...
for i = 1:length(phiVec)
    ...
    for j = 1:length(tt_vec)
        ...
        dlambdadt_vec(j) = helpers{4}(r, C_vec(j,:)', C_vec(j,ind_r_cavity_value)', ind_r_cavity_value, lambda_vec(j), r_needle, kappa, Q_needle, C_oh_inj, epsilon_c_vec(i), epsilon_t);
...
for j = 1:length(phiVec)
    ...
    for i = 1:length(tvec)
        ...
        if tvec(i) > t_injection_end
           ...
           v_tvec(:,i) = helpers{2}(r, ind_r_cavity_tvec, r_needle, theta_vec(j), 0);
        else
           %v_tvec(:,i) = getVelocity.ethanol_velocity_porosity(r, ind_r_cavity_tvec, r_needle, theta_vec(j), Q_needle);
           v_tvec(:,i) = helpers{2}(r, ind_r_cavity_tvec, r_needle, theta_vec(j), Q_needle);
        end
    end
...
if show_mass_conservation == 1
   time_pts_mass_conservation=[1*60, 20*60, 1*60*60, 2*60*60, 4*60*60, 6*60*60];
   t_vec_time_pts = cell(length(phiVec),1);
   for i = 1:length(phiVec)
       t_vec_time_phi = zeros(length(time_pts_mass_conservation),1);
       t_phi = tt{i};
       for j = 1:length(time_pts_mass_conservation)
           t_vec_time_phi(j) = find(t_phi >= time_pts_mass_conservation(j),1);
       end
...
%        time_units = 'hrs';
%    end
   getPlots.plot_mass_conservation_cell(tt, ind_a, r, ind_r_needle, ind_r_cavity, C, phiVec, time_pts_mass_conservation, t_vec_time_pts, 't_units',time_units);
end
