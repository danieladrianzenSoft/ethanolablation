function [x,y] = midpointImplicitSolver(f, J, y0, xSpan, dx, params, options)
...
%     parse(p,varargin{:});
    ...
    for i = 1:numX
        ...
        y(:,i+1) = newtonRaphson(f, J, params, x(i), x(i+1), y(:,i), y_guess, dx, options.tolerance, options.maxIters);
    end
end

function Y = newtonRaphson(f, J, params, xcurr, xnext, ycurr, Y, dx, tolerance, maxIters)
    error = 1;
    %maxIters = 200;
    iter = 1;
    while (error >= tolerance && iter <= maxIters)
        ...
        %params.y = ycurr;
        %jacobian_current = J(params);
        params.y = (1/2) * (ycurr + Y);
        params.x = (xcurr + dx/2);
        jacobian_next = J(params);
