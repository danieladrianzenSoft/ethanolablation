classdef getVelocityLib
    methods(Static)
        function v = velocity_newtonian_fluid_plus_solid(r, ind_r_cavity, r0, theta, Q_needle)
            v = zeros(length(r),1);
            v(1) = 2 * Q_needle ./ ( pi * r0.^2 );
            v(2:ind_r_cavity) = Q_needle ./ ( pi * r(2:ind_r_cavity).^2 );
            v(ind_r_cavity+1:end) = (Q_needle * theta) ./ ( pi * (r(ind_r_cavity+1:end).^2));
