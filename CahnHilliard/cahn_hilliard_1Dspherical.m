function out = cahn_hilliard_1Dspherical(t,u,r,interior_bound,domain_inds,D,gamma) 
...
%         ruDuDr(2:Nr-1,:) = (ru_m(3:Nr,:) - ru_m(1:Nr-2,:)) / (2*dr); % dC/dr = (Ci+1 - Ci-1) / 2dr
%         ruLaplace(2:Nr-1,:) = 2*(ruDuDr(2:Nr-1,:)./r(2:Nr-1)') + (ru_m(1:Nr-2,:) - 2*ru_m(2:Nr-1,:) + ru_m(3:Nr,:)) / dr^2;  % nabla2C = 2/r dC/dr + d2C/dr2
%       
%         ruMu(2:Nr-1,:) = ru_m(2:Nr-1,:).^3 - ru_m(2:Nr-1,:) - gamma*ruLaplace(2:Nr-1,:);
%         ruMuDuDr(2:Nr-1,:) = (ruMu(3:Nr,:) - ruMu(1:Nr-2,:)) / (2*dr);
%         ruMuLaplace(2:Nr-1,:) = 2*ruMuDuDr(2:Nr-1,:)./r(2:Nr-1)' + (ruMu(1:Nr-2,:) - 2*ruMu(2:Nr-1,:) + ru_m(3:Nr,:)) / dr^2;
