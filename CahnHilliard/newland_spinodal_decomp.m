...
arguments
   D double = 1e-7
   gamma double = 0.05
   r (1,:) {mustBeNumeric} = linspace(0,1,300)
   options.EndTime = 30*60
...
interior_bounds = inds_r0(1)-1;
domain_inds = 2:inds_r0(1)-2;
...
%if (options.RunNewSimulation == "true")
    ...
    %temp_t = [0,options.EndTime];
    %temp_rc = 0.3*ones(2,1);
    %temp_t = [];
    rc_spline = spline(t_injection,cavity_radius);
    
    tic
    %[t,ut] = ode15s(@(t,u) cahn_hilliard_1Dspherical(t,u,r,D,gamma), tSpan, IC, opts1);
    %[t,ut] = ode15s(@(t,u) cahn_hilliard_1Dspherical(t,u,r,interior_bounds,domain_inds,D,gamma), tSpan, IC, opts1);
    [t,ut] = ode15s(@(t,u) cahn_hilliard_1Dspherical_rc(t,u,r,rc_spline,D,gamma), tSpan, IC, opts1);
...
plotHeatmapGif_spherical(t,x,y,u,bound_mask,'ShowInteriorBounds','false','GifFileName',options.GifFileName,'FieldOfView',[-0.2,0.2])
