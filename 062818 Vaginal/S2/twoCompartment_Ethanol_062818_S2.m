function twoCompartment_Ethanol_062818_S2
...
%PsurfaceC = C(1,:)/(mean(C(1,:))*1.07);
%PsurfaceC = C(1,:)/(mean(measurements(:,2)));
...
D_OH = 7*10^(-5); %cm^2/s, from "Molecular Dynamics simulation of ethanol/water
%mixtures for structure and diffusion properties, 2005.
D_S = 10.5*10^(-6); %(cm^2/s)
C_0 = 1;
phi = 1;
k_E = 3.0/3600;
...
thickness = [0.90,0.95,0.95,0.98,1.00];
h_S = mean(thickness/10) %convert mm to cm
h_OH = 2; %cm
t = linspace(0,48*60,1001);
