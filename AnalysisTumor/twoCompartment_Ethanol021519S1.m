function [t2,PsurfaceC,tm,MsurfaceC,R2,D_S] = twoCompartment_Ethanol021519S1()

clear
clc

measurements = load('DataVsTime021519S1.csv');
...
%PsurfaceC = C(1,:)/max(measurements(:,2));
%PsurfaceC = C(1,:);
...
figure()
plot(t2/60,PsurfaceC,'LineWidth',2,'Color',[72,61,139]/255)
hold on
plot(measurements(:,1),MsurfaceC,'o','Color',[143,188,143]/255,'MarkerFaceColor',[143,188,143]/255)
%plot(TTemp,ConcTemp,'ro')
legend('Model Predictions','Raman Measurements')
xlabel('Time (mins)','FontSize',18,'FontWeight','Bold')
ylabel('Ethanol concentration (normalized)','FontSize',18,'FontWeight','Bold')
title('Representative Measurements & Predictions of Ethanol Concentration vs Time','FontSize',18,'FontWeight','Bold')
XT = get(gca,'XTick');
set(gca,'FontSize',16)
YT = get(gca,'YTick');
set(gca,'FontSize',16)
...
%thickness = [2.4,2.7,2.3,2.7,2.8];
thickness = [1.9,2.2,2.3,1.8,2.1];
h_S = mean(thickness/10); %convert mm to cm
h_OH = 1; %cm
t = linspace(0,67*60,1001);
